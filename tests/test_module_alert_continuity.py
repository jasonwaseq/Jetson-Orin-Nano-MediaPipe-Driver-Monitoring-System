...
def test_active_updates_are_throttled_and_include_duration():
    ...
    assert router.alerts[0][3]["active_duration_sec"] == 2.5
    assert router.alerts[0][3]["active_update_count"] == 1
...
def test_recovery_event_emits_once_when_condition_clears():
    ...
    assert router.alerts[0][2] == "info"
    assert router.alerts[0][3]["alert_status"] == "recovered"
    assert router.alerts[0][3]["unrecovered"] is False
    assert router.alerts[0][3]["active_duration_sec"] == 5.0


def test_initial_active_event_can_be_enabled():
    router = DummyRouter()
    tracker = AlertContinuityTracker(router, update_interval_sec=1.0, emit_initial_active=True)
    ...
    assert [alert[0] for alert in router.alerts] == ["drowsiness_active"]
